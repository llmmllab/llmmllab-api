...
| `POST /v1/images/3d/parts` | runner pipeline `mesh2parts` (Hunyuan3D-Part / P3-SAM + XPart) | **Mesh-in, meshes-out** — takes a base64-encoded `.glb` and returns four `.glb` files (decomposed, exploded, bbox, gt_bbox). Long-running (several min). |
| `GET  /v1/images/3d/parts/{filename}` | runner pipeline `mesh2parts/files/{filename}` | Streams each part-decomposition `.glb` back through the api. Filename shape: `<id>_<role>.glb` where role ∈ decomposed/exploded/bbox/gt_bbox/input. |

`services/image_service.py` is the single bridge. `generate_image`, `edit_image` acquire a runner server (text/image model handled by `SDCppServerManager`) and hit `/sdapi/v1/txt2img` or `/sdapi/v1/img2img`. `generate_3d` hits `/v1/pipelines/img23d/run` (no server acquisition, the pipeline is in-process on the runner). `generate_3d_parts` hits `/v1/pipelines/mesh2parts/run` with the same in-process pattern but takes a mesh as input instead of an image. `stream_3d_artifact` / `stream_3d_parts_artifact` proxy `GET /v1/pipelines/<name>/files/{filename}` for downloads, with path-traversal-safe regexes on filename. Tests live in `test/unit/test_image_service.py` and `test/unit/test_images_router.py`. CLI test scripts under `scripts/` (see `scripts/README.md`).
